...
*          epsilon value. It can then use this to classify      *
*          the set, complete with functions to check the number *
...
class Classifier:
    ...
    *          by the Classifier class. These attributes include    *
    *          the exmaples set, epsilon value, dimension, target   *
    *          vector, rho, eta, and number of rounds.              *
    *                                                               *
    """""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
    def __init__(self, examples, epsilon):
        ...
        self.is_classified_flag = self.initialize_flag()
        self.upper_bound = self.initialize_upper_bound()
    ...
    * Name: run_round                                               *
    *                                                               *
    * Purpose: This function runs a single round of the MWUA by     *
    *          finding a violating example, generating a gain       *
    *          vector, and updating the weights accordingly.        *
    *          The function also increments the number of rounds    *
    *          taken, but if no violating example is found, rounds  *
    *          is not incremented and the fully classified flag is  *
    *          set.                                                 *
    *                                                               *
    """""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
    def run_round(self):
        ...
        #Get gain_vector from oracle
        gain_vector = self.generate_gain_vector(violating_index)

        #Update target step
        for i in range(0, self.dimension):
            self.target[i] *= 1.0-self.eta*gain_vector[i]
        ...
        #Increment rounds taken and return
        self.rounds += 1
        return
    ...
    * Name: fully_classify                                          *
    *                                                               *
    * Purpose: This function runs the run_round function over and   *
    *          over again either until the set is fully classified  *
    *          or until a given maximum for the number of rounds    *
    *          is reached.                                          *
    *                                                               *
    """""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
    def fully_classify(self, max):
        #Run rounds until fully classified or max is reached
        while self.rounds < max and self.is_classified_flag == False:
            self.run_round()
    ...
    *                                                               *
    * Purpose: This function when given the index of a violating    *
    *          example, generates a gain vector according to        *
    *          formula (-1/rho)*violating_example*label             *
    *                                                               *
    """""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
    def generate_gain_vector(self, violating_index):
        gain_vector = []
        for i in range(0, self.dimension):
